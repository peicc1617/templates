...
function deleteEdge(e) {
    var nodeI = parseInt($(e).attr('nodeI'));
    var nodeJ = parseInt($(e).attr('nodeJ'));
    for (let i = 0; i < GRAPH.edges.length; i++) {
        if (GRAPH.edges[i].nodeI === nodeI && GRAPH.edges[i].nodeJ === nodeJ) {
            //删除对应边
            GRAPH.edges.splice(i, 1);
            i--;
        }
    }
    removeNode(false, resetGRID, 0)
}
